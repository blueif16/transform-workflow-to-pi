...
>
> **Update (2026-06-27) — the security payoff is now DEMONSTRATED end-to-end on cloud.** This section enforces per-node capability removal (read-scope + tool-scope); the `example-academy` 2-node template makes the resulting **lethal-trifecta split** a verifiable fact. It ran GREEN on E2B with **both axes proven from `.pi` telemetry**: a RESEARCH node (external deepwiki MCP, **no shell**, `--exclude-tools bash,edit`) invoked the tool ×2 and wrote a data-only `findings.md`; a BUILD node (shell+`node --test`, **no internet/MCP**) read only that data, wrote impl+test, debugged a real error, and reached **19/19 passing**. No single node holds {private data · untrusted content · exfil channel}, so an injection in fetched content lands where it **cannot escalate** — the orchestrator removed the capability, model-independently. Demo report: `docs/research/2026-06-27-full-agent-sandbox-smoke.md`; honest thesis + citations (Dual-LLM, lethal trifecta, Meta Rule-of-Two, CaMeL): `docs/research/2026-06-27-per-node-capability-isolation.md`. En route this surfaced + fixed two infra bugs (MCP-bridge teardown nonzero exit `fix(compile)` 85517ab; E2B per-command 60 s cap `fix(runner)` edb3a19), each RED→GREEN tested.
...
| `e2b` | isolated BY CONSTRUCTION — agent runs in a REMOTE VM (subtree-namespaced under one shared VM, like daytona); no per-node OS jail INSIDE the VM. Installable `@piflow/e2b` extension, **live-validated 2026-06-27** (full agent: coding + external MCP, both axes green). | n/a (separate machine) | per-VM, **OPEN egress by default** (the MCP-unblock vs daytona's tier-gating) | host-FS breach impossible; node↔node inside VM NOT OS-jailed | `packages/e2b/`, `docs/research/2026-06-27-full-agent-sandbox-smoke.md` |
