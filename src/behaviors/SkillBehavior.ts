...
/**
 * CAPABILITY — self-describing registry sidecars (capability-registry-harness).
 * This file ships ONE base + NINE concrete skill subtypes; the generator
 * (registry/build-registry.mjs) reads `CAPABILITY` (the umbrella) AND every
 * entry of `SKILL_CAPABILITIES` (the 9 bindable skills). The blueprint binds
 * a player's `ultimate` to one skill `id`. EDIT THESE, not capabilities.json.
 */
export const CAPABILITY = {
  kind: 'behavior',
  id: 'SkillBehavior',
  intent: 'Base class for the Q-ultimate: cooldown + charge/sound hooks. Bind a concrete skill subtype (below), not this base.',
  roles: ['player'],
  params: ['id', 'name', 'cooldown', 'chargeAnimKey', 'soundKey'],
  tuning: ['ultimateConfig'],
  abstract: true,
} as const;

export const SKILL_CAPABILITIES = [
  { kind: 'behavior', id: 'DashAttackSkill', intent: 'Linear dash through enemies dealing contact damage along the path.', roles: ['player'], params: ['id', 'name', 'cooldown', 'dashDistance', 'dashDuration', 'hitRange', 'damage'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'AreaDamageSkill', intent: 'AOE burst around the player damaging + knocking back enemies in radius.', roles: ['player'], params: ['id', 'name', 'cooldown', 'effectKey', 'attackRange', 'damage', 'knockbackForceX'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'TargetedExecutionSkill', intent: 'Lock onto the nearest enemy and instantly execute it after a wind-up.', roles: ['player'], params: ['id', 'name', 'cooldown', 'effectKey', 'executionDelay', 'totalDuration', 'screenShake'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'TargetedAOESkill', intent: 'Lock onto an enemy and strike an AOE at its location after a delay (meteor/lightning).', roles: ['player'], params: ['id', 'name', 'cooldown', 'effectKey', 'aoeRadius', 'damage', 'strikeDelay'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'BeamAttackSkill', intent: 'Horizontal beam in the facing direction damaging every enemy along its length.', roles: ['player'], params: ['id', 'name', 'cooldown', 'beamKey', 'beamWidth', 'beamLength', 'damage'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'GroundQuakeSkill', intent: 'Ground slam that damages + knocks back only grounded enemies.', roles: ['player'], params: ['id', 'name', 'cooldown', 'effectKey', 'damage', 'knockbackForceX', 'knockbackForceY'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'BoomerangSkill', intent: 'Returning projectile (hammer/shuriken) that hits on the way out and back.', roles: ['player'], params: ['id', 'name', 'cooldown', 'projectileKey', 'throwSpeed', 'returnSpeed', 'maxDistance'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'MultishotSkill', intent: 'Spread-fire multiple projectiles across an arc in the facing direction.', roles: ['player'], params: ['id', 'name', 'cooldown', 'projectileKey', 'projectileCount', 'spreadAngle', 'projectileSpeed'], tuning: ['ultimateConfig'] },
  { kind: 'behavior', id: 'ArcProjectileSkill', intent: 'Gravity-arced projectile (boulder/grenade) launched on a ballistic path.', roles: ['player'], params: ['id', 'name', 'cooldown', 'projectileKey', 'launchSpeedX', 'launchSpeedY', 'damage'], tuning: ['ultimateConfig'] },
] as const;
