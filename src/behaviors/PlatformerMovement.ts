 ...
 * The generator (registry/build-registry.mjs) globs `behaviors/*.ts` for this
 * const; the blueprint binds entities[].behaviors to `id`. A behavior class
 * exported from the barrel WITHOUT a CAPABILITY fails the stranded-export gate
 * — so "built-but-unwired" is structurally impossible. EDIT THIS, not the JSON.
 */
export const CAPABILITY = {
  kind: 'behavior',
  id: 'PlatformerMovement',
  intent: 'Horizontal walk + jump; air control, optional coyote-time, jump-buffer, double-jump.',
  roles: ['player'],
  params: [
    'walkSpeed', 'jumpPower', 'airControl', 'coyoteTime', 'jumpBufferTime',
    'maxFallSpeed', 'doubleJumpEnabled', 'doubleJumpPower',
  ],
  tuning: ['walkSpeed', 'jumpPower', 'gravityY'],
} as const;
