...
// The engine-accumulated score-ceiling seam (Σ of placed reward values) lives in
// @contract/score and is re-exported so a scorable-placing system reaches it via
// `utils.registerScorable/getMaxScore/resetMaxScore` (this module owns its own
// bus-emitting setScore/addScore below, so those are NOT re-exported here).
export { registerScorable, getMaxScore, resetMaxScore } from '@contract/score';
 ...
 * VISIBLE in the console, never the silent default. DEV DIAGNOSTIC only: it
 * writes nothing to window.__GAME__ / the verify oracle and changes no game
 * behavior. GENERIC: takes a kind + a key, no theme.
 */
