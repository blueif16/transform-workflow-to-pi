...
export const CAPABILITY = {
  kind: 'behavior',
  id: 'PatrolAI',
  intent: 'Walk back and forth between bounds, or turn at cliffs/walls on tilemaps.',
  roles: ['enemy'],
  params: ['speed', 'minX', 'maxX', 'directionChangeDelay', 'cliffCheckDistance', 'detectCliffs'],
  tuning: ['walkSpeed'],
} as const;
...
export class PatrolAI extends BaseBehavior {
  ...
  private checkCliff(owner: Phaser.Physics.Arcade.Sprite, scene: any): boolean {
    ...
    // Cliff detection needs a Tiled TilemapLayer. With programmatic platforms
    // (a StaticGroup) there is no tile grid to probe, so skip it (the enemy
    // patrols within its minX/maxX bounds instead).
    if (typeof groundLayer.worldToTileX !== 'function') return false;
