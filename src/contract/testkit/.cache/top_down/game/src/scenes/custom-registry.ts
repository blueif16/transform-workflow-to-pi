 ...
 * custom entry's factory under its id here; DataTopDownScene RESOLVES "$custom:<id>"
 * behavior bindings and custom[] SYSTEM ids against this registry while building the
 * level from data. This is how the SDK instantiates the level WITHOUT importing any
 ...
 * whatever the game registered. A game's bespoke logic lives ONLY in the
 * game-built project, registered here at boot. (Mirrors platformer's
 * scenes/custom-registry.ts near-verbatim.)
 */
import type { IBehavior } from '../behaviors/IBehavior';
import type { ISceneSystem } from './topdown-data';
