...
export abstract class BasePlayer extends Phaser.Physics.Arcade.Sprite {
  ...
  /**
   * Arrow keys (cursors). The FSM reads BOTH wasdKeys AND cursors for movement,
   * so W5's 'ArrowLeft'/'ArrowRight'/'ArrowUp'/'ArrowDown' inputs and a human's
   * WASD both drive the player.
   */
   ...
   * (the CLAIM the generated event catalog + the wiring gate read). Each is emitted
   * from a real seam in THIS class on the scene's shared bus:
   *   - player.shot    ← shoot()        (a bullet was spawned)        [base:2d]
   *   - player.damaged ← takeDamage()   (non-lethal hit; lethal → scene player.died) [base:2d]
   *   - player.dashed  ← performDash()  (dash start)                  [archetype]
   * Observables stay on the existing __GAME__ adapter, so this surface declares only
   * the PUSH channel + no anchors. (The scene base owns the scene-level moments —
   * player.died / level.statusChanged / enemy.died / score.changed / reward.collected.)
   */
    ...
    return {
      ...
      events: [
        ...
        },
        {
          ...
          payload: '{x,y}',
          scope: 'archetype',
          drivenBy: 'dash input (performDash)',
          expect: 'player.dashed logged',
        },
      ],
    };
  }
  ...
  performDash(dirX: number, dirY: number): void {
    ...
    this.onDashUsed();
    this.dashSound?.play();
    // player.dashed — the standardized dash event on the scene's shared bus, at
    // the real dash-start moment. Defensive: a scene without a bus is a no-op.
    this.bus?.emit('player.dashed', { x: this.x, y: this.y });
