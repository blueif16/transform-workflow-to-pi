...
export abstract class BaseEnemy extends Phaser.Physics.Arcade.Sprite {
   ...
   * (the CLAIM the generated event catalog + the wiring gate read), emitted from a
   * real seam in THIS class on the scene's shared bus:
   *   - enemy.damaged ← takeDamage()  (non-lethal hit; lethal → scene enemy.died) [base:2d]
   * The scene base owns enemy.died (the kill-count seam onEnemyKilled). Observables
   * stay on the existing __GAME__ adapter, so this declares only the PUSH channel.
   */
