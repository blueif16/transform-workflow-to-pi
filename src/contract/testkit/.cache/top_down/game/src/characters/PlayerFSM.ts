...
export class PlayerFSM extends FSM {
  ...
  /** Check if moving left (A key OR ArrowLeft). */
  ...
  /** Check if moving right (D key OR ArrowRight). */
  ...
  /** Check if moving up (W key OR ArrowUp). */
  isMovingUp(): boolean {
    return (
  ...
  /** Check if moving down (S key OR ArrowDown). */
  isMovingDown(): boolean {
    return (
      (this.player.wasdKeys?.S?.isDown ?? false) ||
