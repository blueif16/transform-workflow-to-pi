...
export class ScreenEffectHelper {
  ...
  /**
   * Hit-stop: a brief global time-scale freeze that amplifies an impact (the
   * standard juice primitive — RB §1). Slows the scene's time scale for `durationMs`
   * then restores it. Cosmetic only (it never reads/writes an observed field); the
   * default ~60ms (kill) / ~20ms (hit) follows the research's "kill freezes longer
   * than a hit" rule. GENERIC: a duration param, no game.
   */
  static hitStop(scene: Phaser.Scene, durationMs = 60): void {
    try {
      const anyScene = scene as any;
      // Re-entrancy guard: a second hitStop during a freeze just extends, never
      // stacks the restore (so time.timeScale always returns to 1).
      if (anyScene.__hitStopActive) return;
      anyScene.__hitStopActive = true;
      scene.time.timeScale = 0.05;
      scene.tweens.timeScale = 0.05;
      // Use the real (unscaled) clock to restore so the freeze actually ends.
      window.setTimeout(() => {
        try {
          scene.time.timeScale = 1;
          scene.tweens.timeScale = 1;
        } catch {
          /* scene torn down — ignore */
        }
        anyScene.__hitStopActive = false;
      }, Math.max(0, durationMs));
    } catch (error) {
      console.warn('ScreenEffectHelper.hitStop failed:', error);
    }
  }
