...
import { discoverNamespaces, buildModel, tailNodeOutput } from '../viz-model.mjs';
...
export function App({ config }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const model = await buildModel({ workflowPath: ns.workflow, statusPath: thr.statusPath, run: thr.run, out: config.out });
      ...
      const to = tailNodeOutput({ run: model.run.id, out: config.out, node: outNode });
  ...
  useInput((input, key) => {
    ...
    if (input === 'e') {                                                      // export this run as a Mermaid .mmd snapshot
      ...
      if (m && m.run?.id && nss[ni]) {
        try {
          const file = path.join(nss[ni].dir, config.out, m.run.id, 'graph.mmd');
    ...
    if (focus === 3) {
      ...
      if (key.return) {
        ...
        if (f && nss[ni]) {
          const rel = f.path || `${config.out}/${detail.model.run.id}/${f.rel}`;
          const abs = path.isAbsolute(rel) ? rel : path.join(nss[ni].dir, rel);
