# node: verify-join — memory
<!-- Leg A · OPTIMIZER-FACING. The Hermes fixer / reconcile node READS + UPDATES this from run traces.
     NEVER injected into verify-join's runtime prompt — a node must not see its own failure history.
...
## Current behavior
<!-- what verify-join reliably does now (1–3 lines), updated from traces. -->
...
<!-- the generalized LESSON + WHY (not the diff). Reflect on failures, not successes.
     ...
     the machine `sig:` is what flips a residual LAPSE→SKILL once it recurs; a block with no `sig:` is skipped):
       ### <symptom signature>
       sig: verify-join::<key>          (the machine key = signatureOf output; node::sorted-anomalies|reason)
...
## Active invariants
<!-- hard rules verify-join must keep (e.g. writes only within its owns/readScope). -->

## Open threads
<!-- unresolved; drop each when absorbed. -->

## History
git log --grep '^skillsys(verify-join)'
