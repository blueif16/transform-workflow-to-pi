// `piflow extract <templateDir>` — the FREE DAG preview (no model invoked). The template-era equivalent
...
/** `piflow extract <templateDir>` — print the free DAG preview. Required positional: the template dir. */
export async function runExtractCli(argv: string[]): Promise<void> {
  const dir = argv.find((a) => !a.startsWith('-'));
  if (!dir) {
    process.stderr.write('piflow extract: a template directory is required (piflow extract <templateDir>)\n');
    process.exitCode = 1;
    return;
  }
